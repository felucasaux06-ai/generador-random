    ...
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-white mb-3">{t.chistes.title}</h1>
          <p className="text-zinc-400 text-lg">{t.chistes.subtitle}</p>
        </div>
        ...
        {chisteActual && (
          <div className="bg-zinc-800 border border-zinc-700 rounded-2xl p-6 mb-6 animate-fade-in">
            {/* Setup */}
            <div className="mb-6">
              <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{t.chistes.question}</span>
            ...
            {!revelado ? (
              ...
              <div className="bg-rose-900/20 border border-rose-700/50 rounded-xl p-5 animate-fade-in">
                <span className="text-xs font-bold text-rose-500 uppercase tracking-wider">{t.chistes.punchlineLabel}</span>
            ...
            {revelado && (
              <div className="mt-5 space-y-4 animate-fade-in">
                {/* Votación */}
                <div>
                  <p className="text-zinc-400 text-sm mb-3 text-center">{t.chistes.didLaugh}</p>
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 px-4 rounded-xl font-bold text-lg transition-all border-2 ${
                        votoActual === 'ri'
                          ...
                          : votoActual
                          ? 'border-zinc-700 bg-zinc-700/50 text-zinc-500 cursor-not-allowed'
                          : 'border-zinc-600 bg-zinc-700 text-zinc-300 hover:border-green-500 hover:text-green-400 hover:bg-green-600/10'
                      }`}
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 px-4 rounded-xl font-bold text-lg transition-all border-2 ${
                        votoActual === 'noRi'
                          ...
                          : votoActual
                          ? 'border-zinc-700 bg-zinc-700/50 text-zinc-500 cursor-not-allowed'
                          : 'border-zinc-600 bg-zinc-700 text-zinc-300 hover:border-yellow-500 hover:text-yellow-400 hover:bg-yellow-600/10'
                      }`}
                      aria-label="Votar: no me hizo reír"
                      aria-pressed={votoActual === 'noRi'}
                    >
                      {t.chistes.didntLaugh}
                    </button>
                  </div>

                  {totalVotos > 0 && (
                    <div className="mt-3 p-3 bg-zinc-700/50 rounded-xl" aria-live="polite">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    {copiado ? t.chistes.copied : t.chistes.copy}
                  </button>
                  <button
        ...
        {!chisteActual && (
          <div className="text-center py-16 text-zinc-600">
            ...
            <p className="text-lg">{t.chistes.empty}</p>
          </div>
        )}
